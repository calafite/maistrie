          ...
          public:
            Sieve(Type limit) : limit(limit), isPrime((limit >> 1) + 1, true) {
                ...
                for (Type prime = 3; static_cast<long long>(prime) * prime <= limit; prime += 2) {
                    if (isPrime[static_cast<size_t>(prime >> 1)]) {
                        for (long long multiple = static_cast<long long>(prime) * prime;
                             multiple <= limit; multiple += prime * 2) {
                ...
                if (limit >= 2) {
                    ...
                    for (Type number = 3; number <= limit; number += 2) {
                        if (isPrime[static_cast<size_t>(number >> 1)])
                            primes.pushBack(static_cast<int>(number));
                    }
                }
            }
            ...
            Type count(Type number) const {
                ...
                return static_cast<Type>(
                    std::upper_bound(primes.begin(), primes.end(), static_cast<int>(number)) -
                    primes.begin());
            }
        };
