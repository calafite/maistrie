    ...
    namespace mathematics {
        ...
        template <typename Type> struct LinearSieve {
            ...
            LinearSieve(Type limit) : limit(limit), smallestPrimeFactor((limit >> 1) + 1, 0) {
                if (limit >= 2) {
                    primes.reserve(
                        static_cast<size_t>(limit / std::log(static_cast<double>(limit)) * 1.2) +
                        16);
                    primes.pushBack(2);
                }
                for (Type index = 3; index <= this->limit; index += 2) {
                    ...
                    for (size_t primeIndex = 1; primeIndex < primes.size(); ++primeIndex) {
                        ...
                        if (static_cast<long long>(prime) * index > this->limit)
                            break;
                        smallestPrimeFactor[(index * prime) >> 1] = prime;
                        if (prime == smallestPrimeFactor[halfIndex])
                            break;
                    }
                }
            }
            ...
            Type count(Type number) const {
                ...
                if (number < 2)
                    return 0;
                return static_cast<Type>(std::upper_bound(primes.begin(), primes.end(), number) -
                                         primes.begin());
            }
